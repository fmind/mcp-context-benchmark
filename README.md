...
| What does a CLI skill cost in a real host, next to MCP? | Claude Code 2.1.280, additions over one baseline: `gh` skill 26; 81 `fmind/dot` skills 644 with the default listing budget (66 of 92 entries lose descriptions) or 1,886 with every description; GitHub MCP eager 11,695; tool search 1,285 | `o200k_base` reference tokens, system + messages + tools | [cli-results.json](cli-results.json) |
| What do CLI and MCP return for the same data? | `gh --help` 666, `gh api --help` 1,882; issue #2275: `gh issue view` 714, MCP `issue_read` 957, `--json` without body 42; pinned `tools.go`: `gh api` default 15,708 (base64 JSON), raw 4,265, MCP `get_file_contents` 4,298 | `o200k_base` tokens of raw output text | [data/cli-outputs.json](data/cli-outputs.json) |
