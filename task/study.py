...
# ///
# ruff: noqa: T201
"""Three-strategy study; offline validation unless --execute is explicit."""
...
def allowed(case: dict, name: str, arguments: dict) -> bool:
    ...
    if name == "get_file_contents" and "sha" in normalized:
        sha = normalized.pop("sha")
        if sha != case["arguments"]["ref"] or normalized.get("ref", sha) != sha:
            return False
        normalized["ref"] = sha
    return name == case["tool"] and normalized == case["arguments"]
...
async def main() -> None:
    ...
    jobs = schedule(cases)
    if not args.execute:
        print(f"Offline validation passed: {len(cases)} frozen cases, {len(jobs)} planned trials; no inference.")
        return
    ...
    identity = {
        ...
        "system": SYSTEM,
        "fixed_tools": FIXED,
        "authority_policy": "pinned-ref-or-sha-v2",
    ...
    if args.resume_from:
        ...
        if previous.get("authority_policy") != identity["authority_policy"]:
            raise ValueError("Cannot resume across authority policy revisions; preserve the historical run")
        if (
